...
dados_brutos.columns = dados_brutos.columns.str.replace(" \(US\$\)", "", regex=True).str.replace(" \(KG\)| \(Kg\)", "", regex=True)

# LEITURA DE ARQUIVO CONTENDO DADOS SOBRE QUANTIDADE EM LITROS E OS VALORES DE VENDAS
dados_uva = pd.read_csv("ExpUva_1970-2022.csv", encoding="utf-8", sep=";", thousands=".")
dados_uva = dados_uva.drop(columns="ID")
dados_uva.index = dados_uva['País']
dados_uva.columns = dados_uva.columns.str.replace(" \(US\$\)", "", regex=True).str.replace(" \(KG\)| \(Kg\)", "", regex=True)
...
dados_uva = dados_uva.loc[:, ~dados_uva.columns.str.contains('|'.join(excluir_anos))]
...
#for i in range(len(lista_valor_anos_recentes)-1):
#    dados_brutos["tendencia_crescente"] = np.where((dados_brutos[i] < dados_brutos[i+1], 1, 0))

#dados_brutos['tendencia crescimento'] =
#dados_brutos["tendencia_crescente"] = np.where((dados_brutos['VALOR 2017'] < dados_brutos['VALOR 2018']) & (dados_brutos['VALOR 2018'] < dados_brutos['VALOR 2019']), 1, 0)
#dados_brutos["tendencia_crescente"]

  #dados_brutos['tendencia crescimento'] =
#dados_brutos["tendencia_oscilante"] = np.where(((dados_brutos['VALOR 2017'] > dados_brutos['VALOR 2018']) & (dados_brutos['VALOR 2018'] < dados_brutos['VALOR 2019']) |
#(dados_brutos['VALOR 2017'] < dados_brutos['VALOR 2018']) & (dados_brutos['VALOR 2018'] > dados_brutos['VALOR 2019'])), 1, 0)
#dados_brutos["tendencia_oscilante"]

  #dados_brutos['tendencia crescimento'] =
# dados_brutos["tendencia_decrescente"] = np.where((dados_brutos['VALOR 2017'] > dados_brutos['VALOR 2018']) & (dados_brutos['VALOR 2018'] > dados_brutos['VALOR 2019']), 1, 0)
#dados_brutos["tendencia_decrescente"]

# COLOCANDO A COLUNA "PAÍS" COMO INDEX

#ordenados_por_valor_total = dados_brutos.sort_values(by = 'Media_3_anos_recentes', ascending=False)
#ordenados_por_valor_total.index = ordenados_por_valor_total['PAÍS']

# DESCOBRINDO O VALOR DAS EXPORTAÇÕES EM US$ NOS ÚLTIMOS 3 ANOS #
...
#dados_brutos_aberto = dados_brutos.reset_index().melt(id_vars=["index"], value_vars=dados_brutos.columns)
#dados_brutos_aberto.columns = ["Lista_paises", 'VALOR 2005', 'VALOR 2006', 'VALOR 2007', 'VALOR 2008', 'VALOR 2009', 'VALOR 2010', 'VALOR 2011', 'VALOR 2012', 'VALOR 2013', 'VALOR 2014', 'VALOR 2015', 'VALOR 2016', 'VALOR 2017', 'VALOR 2018', 'VALOR 2019']
#dados_brutos_aberto.head()
...
dados_uva["QUANTIDADE TOTAL"] = dados_uva.filter(regex=r'QUANTIDADE 20').sum(axis=1)
dados_uva["VALOR TOTAL"] = dados_uva.filter(regex=r'VALOR 20').sum(axis=1)

lista_valor_anos_recentes = ["VALOR " + s for s in lista_anos_curto]
lista_qtde_anos_recentes = ["QUANTIDADE " + s for s in lista_anos_curto]

dados_uva["Valor_Media_anos_recentes"] = dados_uva[lista_valor_anos_recentes].mean(axis=1).astype(int)
dados_uva["Valor_Soma_anos_recentes"] = dados_uva[lista_valor_anos_recentes].sum(axis=1).astype(int)

dados_uva["Qtde_Media_anos_recentes"] = dados_uva[lista_qtde_anos_recentes].mean(axis=1).astype(int)
dados_uva["Qtde_Soma_anos_recentes"] = dados_uva[lista_qtde_anos_recentes].sum(axis=1).astype(int)

# DESCOBRINDO QUAL PAÍS MAIS IMPORTOU VINHOS NOS ÚLTIMOS 15 ANOS #

axis = dados_uva.sort_values(by = 'QUANTIDADE TOTAL', ascending=False).head(quantidade_paises).plot(x="País", y="QUANTIDADE TOTAL", kind="bar", figsize=(18,6))
axis.yaxis.set_major_formatter(ticker.StrMethodFormatter("{x:,.2f}"))
#axis.bar(x = 2, height = 1, width = 3)
plt.title(f"QUANTIDADE TOTAL DE UVA EXPORTADA EM KG ({primeiro_ano}-{ano_recente})")
plt.xticks(rotation = 45)
plt.show()

axis = dados_uva.sort_values(by = 'VALOR TOTAL', ascending=False).head(quantidade_paises).plot(x="País", y="VALOR TOTAL", kind="bar", figsize=(18,6))
axis.yaxis.set_major_formatter(ticker.StrMethodFormatter("{x:,.2f}"))
#axis.bar(x = 2, height = 1, width = 3)
plt.title(f"VALOR TOTAL DE KG EXPORTADA EM US$ ({primeiro_ano}-{ano_recente})")
plt.xticks(rotation = 45)
plt.show()

lista_valor_periodo_curto_uva = dados_uva.sort_values(by = 'Valor_Soma_anos_recentes', ascending=False).head(quantidade_paises)
lista_valor_periodo_curto_uva = lista_valor_periodo_curto_uva['País'].to_list()

lista_valor_periodo_curto_uva_T = dados_uva.loc[:, dados_uva.columns.str.contains('|'.join(lista_anos_curto))]
lista_valor_periodo_curto_uva_T = lista_valor_periodo_curto_uva_T.filter(like='VALOR 20', axis=1).T.reset_index().melt(id_vars=["index"], value_vars=lista_valor_periodo_curto_uva_T.filter(like='VALOR', axis=1).T.columns)
lista_valor_periodo_curto_uva_T.columns = ["Ano", "PAÍS", "VALOR ANO"]
lista_valor_periodo_curto_uva_T = lista_valor_periodo_curto_uva_T.loc[lista_valor_periodo_curto_uva_T['PAÍS'].isin(lista_valor_periodo_curto_uva), :]


lista_valor_periodo_curto_uva_T["Ano"] = lista_valor_periodo_curto_uva_T["Ano"].str.replace('VALOR ', '', regex=False)
lista_valor_periodo_curto_uva_T.index = lista_valor_periodo_curto_uva_T["Ano"]

lista_qtde_periodo_curto_uva = dados_uva.sort_values(by = 'Qtde_Soma_anos_recentes', ascending=False).head(quantidade_paises)
lista_qtde_periodo_curto_uva = lista_qtde_periodo_curto_uva['País'].to_list()

lista_qtde_periodo_curto_uva_T = dados_uva.loc[:, dados_uva.columns.str.contains('|'.join(lista_anos_curto))]
lista_qtde_periodo_curto_uva_T = lista_qtde_periodo_curto_uva_T.filter(like='QUANTIDADE 20', axis=1).T.reset_index().melt(id_vars=["index"], value_vars=lista_qtde_periodo_curto_uva_T.filter(like='QUANTIDADE', axis=1).T.columns)
lista_qtde_periodo_curto_uva_T.columns = ["Ano", "PAÍS", "QUANTIDADE ANO"]
lista_qtde_periodo_curto_uva_T = lista_qtde_periodo_curto_uva_T.loc[lista_qtde_periodo_curto_uva_T['PAÍS'].isin(lista_qtde_periodo_curto_uva), :]


lista_qtde_periodo_curto_uva_T["Ano"] = lista_qtde_periodo_curto_uva_T["Ano"].str.replace('QUANTIDADE ', '', regex=False)
lista_qtde_periodo_curto_uva_T.index = lista_qtde_periodo_curto_uva_T["Ano"]

lista_valor_periodo_longo_uva = dados_uva.sort_values(by = 'VALOR TOTAL', ascending=False).head(quantidade_paises)
lista_valor_periodo_longo_uva = lista_valor_periodo_longo_uva['País'].to_list()

lista_valor_periodo_longo_uva_T = dados_uva.filter(like='VALOR 20', axis=1).T.reset_index().melt(id_vars=["index"], value_vars=dados_uva.filter(like='VALOR', axis=1).T.columns)
lista_valor_periodo_longo_uva_T.columns = ["Ano", "PAÍS", "VALOR ANO"]
lista_valor_periodo_longo_uva_T = lista_valor_periodo_longo_uva_T.loc[lista_valor_periodo_longo_uva_T['PAÍS'].isin(lista_valor_periodo_longo), :]

lista_valor_periodo_longo_uva_T["Ano"] = lista_valor_periodo_longo_uva_T["Ano"].str.replace('VALOR ', '', regex=False)
lista_valor_periodo_longo_uva_T.index = lista_valor_periodo_longo_uva_T["Ano"]

lista_qtde_periodo_longo_uva = dados_uva.sort_values(by = 'QUANTIDADE TOTAL', ascending=False).head(quantidade_paises)
lista_qtde_periodo_longo_uva = lista_qtde_periodo_longo_uva['País'].to_list()

lista_qtde_periodo_longo_uva_T = dados_uva.filter(like='QUANTIDADE 20', axis=1).T.reset_index().melt(id_vars=["index"], value_vars=dados_uva.filter(like='QUANTIDADE', axis=1).T.columns)
lista_qtde_periodo_longo_uva_T.columns = ["Ano", "PAÍS", "QUANTIDADE ANO"]
lista_qtde_periodo_longo_uva_T = lista_qtde_periodo_longo_uva_T.loc[lista_qtde_periodo_longo_uva_T['PAÍS'].isin(lista_qtde_periodo_longo_uva), :]

lista_qtde_periodo_longo_uva_T["Ano"] = lista_qtde_periodo_longo_uva_T["Ano"].str.replace('QUANTIDADE ', '', regex=False)
lista_qtde_periodo_longo_uva_T.index = lista_qtde_periodo_longo_uva_T["Ano"]

plt.figure(figsize=(18,12))
plot_2 = sns.lineplot(data=lista_valor_periodo_curto_uva_T, x="Ano", y="VALOR ANO", hue="PAÍS")
plot_2.yaxis.set_major_formatter(ticker.StrMethodFormatter("{x:,.2f}"),)
plt.title(f"VALOR ANUAL DAS EXPORTAÇÕES EM US$ ({lista_anos_curto[0]}-{ano_recente})")
plot_2.set_xticks(lista_anos_curto)
plot_2.set_xticklabels(lista_anos_curto, rotation=45)
...
plt.figure(figsize=(18,12))
plot_2 = sns.lineplot(data=lista_qtde_periodo_curto_uva_T, x="Ano", y="QUANTIDADE ANO", hue="PAÍS")
plot_2.yaxis.set_major_formatter(ticker.StrMethodFormatter("{x:,.2f}"),)
plt.title(f"QUANTIDADE ANUAL DAS EXPORTAÇÕES EM LITROS ({lista_anos_curto[0]}-{ano_recente})")
plot_2.set_xticks(lista_anos_curto)
plot_2.set_xticklabels(lista_anos_curto, rotation=45)
